...
create_collector_pr() {
  ...
  local branch="create-pull-request/update-injector"
  local message="Update opentelemetry-injector to latest"
  ...
  if ! git diff --exit-code >/dev/null 2>&1; then
    create_pr_with_changelog "$repo" "$repo_url" "$branch" "$message" \
      "update-injector-${tag}" "packaging" "Update OpenTelemetry injector to ${tag}"
  fi
}
