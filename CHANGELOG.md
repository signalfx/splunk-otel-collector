...
- (Contrib) `connector/servicegraph`: remove deprecated warning log about metrics exporter logical ([#45177](https://github.com/open-telemetry/opentelemetry-collector-contrib/pull/45177))
- (Contrib) `extension/googlecloudlogentry_encoding`: Parse Cloud DNS Query logs into log record attributes instead of placing it in the body as is. ([#44561](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/44561))
- (Contrib) `processor/tail_sampling`: Add support for caching the policy name involved in a sampling decision. ([#45040](https://github.com/open-telemetry/opentelemetry-collector-contrib/pull/45040))
...
- (Contrib) `receiver/systemd`: Rename `systemd.unit.cpu.time` metric to `systemd.service.cpu.time` ([#44916](https://github.com/open-telemetry/opentelemetry-collector-contrib/pull/44916))
...
- (Contrib) `cmd/schemagen`: Introduce script that generates configuration schemas for collector components based on go structs. ([#42214](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/42214))
...
- (Splunk) `packaging`: Update Splunk OpenTelemetry Node.js agent to v4.2.0 ([#7042](https://github.com/signalfx/splunk-otel-collector/pull/7042))
...
- (Core) `all`: Update semconv import to 1.38.0 ([#14305](https://github.com/open-telemetry/opentelemetry-collector/pull/14305))
- (Core) `exporter/nop`: Add profiles support to nop exporter ([#14331](https://github.com/open-telemetry/opentelemetry-collector/issues/14331))
- (Core) `pkg/pdata`: Optimize the size and pointer bytes for pdata structs ([#14339](https://github.com/open-telemetry/opentelemetry-collector/pull/14339))
- (Core) `pkg/pdata`: Avoid using interfaces/oneof like style for optional fields ([#14333](https://github.com/open-telemetry/opentelemetry-collector/pull/14333))
- (Contrib) `exporter/azureblob`: Add `time_parser_ranges` option to allow selective time parsing of blob name substrings ([#44650](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/44650))
...
- (Contrib) `exporter/coralogix`: Exposed a new field to set `grpc-accept-encoding`. `gzip` will be used by default. ([#45191](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/45191))
- (Contrib) `exporter/coralogix`: Improve log messages when a partial success happens for traces. ([#44926](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/44926))
...
- (Contrib) `exporter/elasticsearch`: Add support for extra query parameters to the outgoing bulk request ([#44480](https://github.com/open-telemetry/opentelemetry-collector-contrib/pull/44480))
- (Contrib) `exporter/kafka`: Make `max_message_bytes` and `flush_max_messages` unconditional in franz-go producer. Changed `flush_max_messages` default from 0 to 10000 to match franz-go default. ([#44840](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/44840))
- (Contrib) `extension/awslogs_encoding`: Enhance VPC flow logs encoding extension with CloudWatch logs support ([#44710](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/44710))
- (Contrib) `extension/azure_encoding`: Add processing for AppService, CDN, FrontDoor and Recommendation logs records ([#41725](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/41725))
- (Contrib) `extension/googlecloudlogentry_encoding`: Add support for Passthrough External and Internal Network Load Balancer logs ([#44524](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/44524))
...
- (Contrib) `processor/geoip`: Bump oschwald/geoip2 to v2 ([#44687](https://github.com/open-telemetry/opentelemetry-collector-contrib/pull/44687))
...
- (Contrib) `receiver/awslambda`: Add support for AWS Lambda receiver to trigger by CloudWatch logs subscription filters for Lambda ([#43504](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/43504))
- (Contrib) `receiver/awslambda`: Add S3 failure replay support to AWS Lambda receiver ([#43504](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/43504))
...
- (Contrib) `receiver/github`: Add `merged_pr_lookback_days` configuration to limit historical PR queries and reduce API usage ([#43388](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/43388))
...
- (Contrib) `receiver/systemd`: Add metric for number of times a service has restarted. ([#45071](https://github.com/open-telemetry/opentelemetry-collector-contrib/pull/45071))
...
- (Contrib) `exporter/elasticsearch`: Deduplicate attribute keys from non-compliant SDKs in otel mapping mode ([#39304](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/39304))
...
- (Contrib) `pkg/translator/azurelogs`: Fix missing data when ingesting Azure logs without properties field. ([#44222](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/44222))
- (Contrib) `receiver/awss3`: Fix data loss when SQS messages contain multiple S3 object notifications and some fail to process ([#45153](https://github.com/open-telemetry/opentelemetry-collector-contrib/pull/45153))
