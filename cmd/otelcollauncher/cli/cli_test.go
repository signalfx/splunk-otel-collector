...
func TestDispatch(t *testing.T) {
	tests := map[string]struct {
		...
		"single dash flag passes through": {
			args:   []string{"-x"},
			wantOK: false,
		},
		"double dash flag passes through": {
			args:   []string{"--config", "/x"},
			wantOK: false,
		},
		"-h passes through at the top level": {
			args:   []string{"-h"},
			wantOK: false,
		},
		"--help passes through at the top level": {
			args:   []string{"--help"},
			wantOK: false,
		},
...
func TestDispatchBareHelp(t *testing.T) {
	f := &fakeFamily{verbs: verbInfos("start", "stop")}
	outBuf := &bytes.Buffer{}
	streams := IO{Out: outBuf, Err: &bytes.Buffer{}}

	code, ok := Dispatch([]Family{f}, []string{"help"}, streams)
...
func TestDispatchHelpWithTrailingArgsIgnored(t *testing.T) {
	...
	code, ok := Dispatch([]Family{f}, []string{"help", "-x"}, IO{Out: outBuf, Err: &bytes.Buffer{}})
...
func TestDispatchHelpForKnownVerb(t *testing.T) {
	f := &fakeFamily{verbs: []VerbInfo{
		{Name: "start", Help: "start the collector", Usage: "Starts the collector or supervisor process.\nExtended usage details go here."},
		{Name: "stop", Help: "stop the collector"},
	}}
	outBuf := &bytes.Buffer{}
	streams := IO{Out: outBuf, Err: &bytes.Buffer{}}

	code, ok := Dispatch([]Family{f}, []string{"help", "start"}, streams)

	assert.True(t, ok)
	assert.Equal(t, ExitOK, code)
	assert.Empty(t, f.calls, "help start must not dispatch to the family")
	assert.Contains(t, outBuf.String(), "otelcollauncher start")
	assert.Contains(t, outBuf.String(), "start the collector")
	assert.Contains(t, outBuf.String(), "Extended usage details go here")
	assert.NotContains(t, outBuf.String(), "stop the collector", "must not show unrelated verbs")
}

func TestDispatchHelpForVerbWithoutUsageFallsBackToHelp(t *testing.T) {
	f := &fakeFamily{verbs: verbInfos("start")}
	outBuf := &bytes.Buffer{}

	code, ok := Dispatch([]Family{f}, []string{"help", "start"}, IO{Out: outBuf, Err: &bytes.Buffer{}})

	assert.True(t, ok)
	assert.Equal(t, ExitOK, code)
	assert.Contains(t, outBuf.String(), "start help text")
}

func TestDispatchHelpForUnknownVerbFallsBackToTopLevelList(t *testing.T) {
	f := &fakeFamily{verbs: verbInfos("start", "stop")}
	outBuf := &bytes.Buffer{}
	errBuf := &bytes.Buffer{}

	code, ok := Dispatch([]Family{f}, []string{"help", "bogus"}, IO{Out: outBuf, Err: errBuf})

	assert.True(t, ok)
	assert.Equal(t, ExitUsage, code)
	assert.Contains(t, errBuf.String(), "bogus")
	assert.Contains(t, outBuf.String(), "start")
	assert.Contains(t, outBuf.String(), "stop")
}

func TestDispatchVerbHelpFlag(t *testing.T) {
	for _, helpArg := range []string{"-h", "--help"} {
		t.Run(helpArg, func(t *testing.T) {
			f := &fakeFamily{verbs: []VerbInfo{
				{Name: "start", Help: "start the collector", Usage: "Extended start usage."},
			}}
			outBuf := &bytes.Buffer{}
			streams := IO{Out: outBuf, Err: &bytes.Buffer{}}

			code, ok := Dispatch([]Family{f}, []string{"start", helpArg}, streams)

			assert.True(t, ok)
			assert.Equal(t, ExitOK, code)
			assert.Empty(t, f.calls, "start "+helpArg+" must not dispatch to the family")
			assert.Contains(t, outBuf.String(), "otelcollauncher start")
			assert.Contains(t, outBuf.String(), "Extended start usage")
		})
	}
}

func TestDispatchVerbHelpFlagOnlyChecksImmediateNextToken(t *testing.T) {
	f := &fakeFamily{verbs: verbInfos("start")}
	streams := IO{Out: &bytes.Buffer{}, Err: &bytes.Buffer{}}

	code, ok := Dispatch([]Family{f}, []string{"start", "--config", "/x", "--help"}, streams)

	assert.True(t, ok)
	assert.Equal(t, ExitOK, code)
	require.Len(t, f.calls, 1)
	assert.Equal(t, []string{"--config", "/x", "--help"}, f.calls[0].args)
}
