...
- `splunk_config_override_list_merge`: This variable is used to configure `list_merge` argument for `splunk_config_override`. When the hashes to merge contain arrays/lists. (**default:** `replace` the possible values are replace, keep, append, prepend, append_rp or prepend_rp)
