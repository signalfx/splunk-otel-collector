...
```terminal
$> ansible-playbook playbook.yaml -e collector_start_service=false
```
...
- `collector_splunk_access_token` (**Required**): The Splunk access token to
  authenticate requests.

- `collector_splunk_realm`: Which realm to send the data to. The `SPLUNK_REALM`
...
- `collector_splunk_otel_collector_config`: Splunk OTel Collector config YAML file. Can be set to 
...
- `collector_splunk_config_override`: Custom Splunk OTel Collector config that will be merged into the default config.

- `collector_splunk_config_override_list_merge`: This variable is used to configure `list_merge` option for merging lists in `collector_splunk_config_override` with lists in default config. Allowed options are `replace`, `keep`, `append`, `prepend`, `append_rp` or `prepend_rp`. More details: https://docs.ansible.com/ansible/latest/user_guide/playbooks_filters.html#combining-hashes-dictionaries. (**default:** `replace`)

- `collector_splunk_otel_collector_config_source`: Source path to a Splunk OTel Collector config YAML 
  file on your control host that will be uploaded and set in place of
  `collector_splunk_otel_collector_config` in remote hosts. Can be used to submit a custom collector 
  config, e.g. `./custom_collector_config.yaml`. (**default:** `""` meaning 
  that nothing will be copied and existing `collector_splunk_otel_collector_config` will be used)

- `collector_splunk_bundle_dir`: The path to the [Smart Agent bundle directory](
...
- `collector_splunk_collectd_dir`: The path to the collectd config directory for the
...
- `collector_splunk_service_user` and `collector_splunk_service_group` (Linux only): Set the user/group
  ownership for the collector service. The user/group will be created if they
  do not exist. (**default:** `splunk-otel-collector`)

- `collector_splunk_otel_collector_proxy_http` and `collector_splunk_otel_collector_proxy_https`
...
- `collector_splunk_otel_collector_no_proxy` (Linux only): Set the ip and/or hosts that
  will not use `collector_splunk_otel_collector_proxy_http` or
  `collector_splunk_otel_collector_proxy_https`. This variable is only used if
  `collector_splunk_otel_collector_proxy_http` or `collector_splunk_otel_collector_proxy_https` is
  defined. (**default:** `localhost,127.0.0.1,::1`)

- `collector_splunk_otel_collector_command_line_args`: Command-line arguments to pass to the
  ...
  Example:
  ```yaml
  collector_splunk_otel_collector_command_line_args: "--discovery --set=processors.batch.timeout=10s"
  ```

- `collector_splunk_memory_total_mib`: Amount of memory in MiB allocated to the Splunk OTel 
  Collector. (**default:** `512`)

- `collector_gomemlimit`: The `GOMEMLIMIT` environment variable is introduced for the Splunk Otel Collector version >=0.97.0, allowing the limitation of memory usage in the GO runtime. This feature can help enhance GC (Garbage Collection) related performance and prevent GC related Out of Memory (OOM) situations.

- `collector_splunk_listen_interface`: The network interface the collector receivers will listen on.
  (**default** `0.0.0.0`).

- `collector_splunk_skip_repo` (Linux only): If installing the collector from a custom or self-hosted
  apt/yum repo, set to `true` to skip the installation of the default repo
  (**default:** `false`)

- `collector_start_service`: Whether to restart the services installed by the playbook. (**default:** true)

- `collector_splunk_otel_collector_additional_env_vars`: Dictionary of additional environment variables
  ...
  variables:
  ```yaml
  collector_splunk_otel_collector_additional_env_vars:
...
- `collector_install_splunk_otel_auto_instrumentation` (Linux only): Whether to
...
- `collector_splunk_otel_auto_instrumentation_sdks`: List of Splunk OpenTelemetry Auto
...
- `collector_splunk_otel_auto_instrumentation_version` (Linux only): Version of the
...
- `collector_splunk_otel_auto_instrumentation_systemd` (Linux only): By default, the
...
- `collector_splunk_otel_auto_instrumentation_ld_so_preload` (Linux only): Configure this
  variable to include additional library paths, e.g. `/path/to/my.library.so`,
  to `/etc/ld.so.preload`. (**default:** ``)

- `collector_splunk_otel_auto_instrumentation_java_agent_jar` (Linux only): Path to the
...
- `collector_splunk_otel_auto_instrumentation_npm_path`: If the
  `collector_splunk_otel_auto_instrumentation_sdks` option includes `nodejs`, the Splunk
...
- `collector_splunk_otel_auto_instrumentation_service_name` (Linux only): Explicitly set
...
- **DEPRECATED** `collector_splunk_otel_auto_instrumentation_generate_service_name`
  ...
  `collector_splunk_otel_auto_instrumentation_version` is < `0.87.0`.
  (**default:** `true`)

- **DEPRECATED** `collector_splunk_otel_auto_instrumentation_disable_telemetry`
  ...
  applicable if `collector_splunk_otel_auto_instrumentation_version` is < `0.87.0`.
  (**default:** `false`)

- `collector_splunk_otel_auto_instrumentation_enable_profiler` (Linux only): Enable or
  disable AlwaysOn CPU Profiling. (**default**: `false`)

- `collector_splunk_otel_auto_instrumentation_enable_profiler_memory` (Linux only):
  Enable or disable AlwaysOn Memory Profiling. (**default:** `false`)

- `collector_splunk_otel_auto_instrumentation_enable_metrics` (Linux only): Enable or
  disable exporting instrumentation metrics. (**default**: `false`)

- `collector_splunk_otel_auto_instrumentation_otlp_endpoint` (Linux only): Set the OTLP
  ...
  `collector_splunk_otel_auto_instrumentation_version` is `latest` or >= `0.87.0`.
  (**default:** ``, i.e. defer to the default `OTEL_EXPORTER_OTLP_ENDPOINT`
  value for each activated SDK)

- `collector_splunk_otel_auto_instrumentation_otlp_endpoint_protocol` (Linux only): Set
  ...
  variable. Only applicable if `collector_splunk_otel_auto_instrumentation_version` is
  `latest` or >= `0.104.0`.
  (**default:** ``, i.e. defer to the default `OTEL_EXPORTER_OTLP_PROTOCOL`
  value for each activated SDK)

- `collector_splunk_otel_auto_instrumentation_metrics_exporter` (Linux only):
  ...
  `collector_splunk_otel_auto_instrumentation_version` is `latest` or >= `0.104.0`.
  (**default:** ```, i.e. defer to the default `OTEL_METRICS_EXPORTER` value
  for each activated SDK)

- `collector_splunk_otel_auto_instrumentation_logs_exporter` (Linux only):
  ...
  applicable if `collector_splunk_otel_auto_instrumentation_version` is `latest` or >=
  `0.108.0`.
...
`collector_splunk_dotnet_auto_instrumentation_additional_options` option (see below for
...
- `collector_install_splunk_dotnet_auto_instrumentation` (Windows only): Whether to
...
- `collector_splunk_dotnet_auto_instrumentation_version` (Windows only): Version of the
  ...
  downloaded from
  `https://github.com/signalfx/splunk-otel-dotnet/releases/download/v{{ collector_splunk_dotnet_auto_instrumentation_version }}`.
  (**default:** `latest`)

- `collector_splunk_dotnet_auto_instrumentation_url` (Windows only): Specify the
  ...
  `collector_splunk_dotnet_auto_instrumentation_version` option is ignored.
  (**default:** ``)

- `collector_splunk_dotnet_auto_instrumentation_github_token` (Windows only): Specify
  ...
  `collector_splunk_dotnet_auto_instrumentation_version` is `latest` or when not using
  `collector_splunk_dotnet_auto_instrumentation_url` since unauthenticated requests
  are [rate-limited](https://docs.github.com/en/rest/rate-limit) by GitHub.
  (**default:** ``)

- `collector_splunk_dotnet_auto_instrumentation_iisreset` (Windows only): By default,
...
- `collector_splunk_dotnet_auto_instrumentation_system_wide` (Windows only): By
  ...
  `collector_splunk_dotnet_auto_instrumentation_additional_options` option to
  ...
  OTEL_RESOURCE_ATTRIBUTES: "deployment.environment={{ collector_splunk_dotnet_auto_instrumentation_environment }},{{ collector_splunk_otel_auto_instrumentation_resource_attributes }},splunk.zc.method=splunk-otel-dotnet-1.8.0"
  ...
  from the `collector_splunk_dotnet_auto_instrumentation_additional_options` option to
  the
  `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\Environment`
  registry key to enable/configure auto instrumentation for ***all*** .NET
  applications on the node. (**default:** `false`)

- `collector_splunk_dotnet_auto_instrumentation_environment` (Windows only): Configure
...
- `collector_splunk_dotnet_auto_instrumentation_service_name` (Windows only): Configure
...
- `collector_splunk_dotnet_auto_instrumentation_enable_profiler` (Windows only): Set
...
- `collector_splunk_dotnet_auto_instrumentation_enable_profiler_memory` (Windows only):
...
- `collector_splunk_dotnet_auto_instrumentation_additional_options` (Windows only):
  Dictionary of environment variables to be added to the Windows registry
  ***in addition*** to the options above. (**default:** `{}`)

  For example:
  ```yaml
  collector_splunk_dotnet_auto_instrumentation_additional_options:
