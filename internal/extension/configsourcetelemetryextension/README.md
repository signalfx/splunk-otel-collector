...
`otelcol_splunk_config_source_usage` observable gauge against the **service's own**
`MeterProvider`. The metric then appears at the collector's internal Prometheus
endpoint (`/metrics`) and can be automatically scraped and forwarded to your observability
backend using a `prometheus` receiver. If you are using the default configuration, 
this is handled by the `prometheus/internal` receiver pipeline.
...
service:
  extensions: [config_source_telemetry]
```
...
|-------------------------------------|-------|----------------------|-------------|
| `otelcol_splunk_config_source_usage` | Gauge | `config_source_type` | Emitted with value `1` for each custom config source that is present in the collector config. No datapoint is emitted for config sources that are not in use. |
...
```
# HELP otelcol_splunk_config_source_usage Indicates whether a custom config source is in use (1 = in use)
# TYPE otelcol_splunk_config_source_usage gauge
otelcol_splunk_config_source_usage{config_source_type="include"} 1
otelcol_splunk_config_source_usage{config_source_type="vault"} 1
```
