...
func (b sparkClusterMetricsBuilder) buildMetrics(
	builder *metadata.MetricsBuilder,
	now pcommon.Timestamp,
	clusters []cluster,
	pipelines []pipelineSummary,
) ([]pmetric.Metric, error) {
	...
	for clstr, clusterMetric := range clusterMetrics {
		...
		b.buildMetricsForCluster(builder, clusterMetric, now, clstr.ClusterID, pipelineRef)
		b.buildClusterTimers(builder, clusterMetric, now, clstr.ClusterName)
		otelHistos := b.sparkClusterHistosToOtelHistos(clusterMetric, now)
