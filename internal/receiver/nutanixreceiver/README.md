...
and retries. Each API family uses the version compiled into its released client.

The receiver leaves generated-client debug logging disabled. Request logging
behavior otherwise follows the released Nutanix SDK used for each API family;
receiver warnings and errors use the collector logger.
...
collector; these paths are not hardcoded in the receiver.
...
| --- | --- | --- |
| `nutanix_cluster_stats_<stat>` | cluster stats | `nutanix.cluster.stat.<stat_name>` with `nutanix.stat.name` |
| `nutanix_cluster_usage_stats_<stat>` | cluster usage stats | `nutanix.cluster.stat.<stat_name>` with v4 stat names |
| `nutanix_host_stats_<stat>` | host stats | `nutanix.host.stat.<stat_name>` with `nutanix.stat.name` |
| `nutanix_host_usage_stats_<stat>` | host usage stats | `nutanix.host.stat.<stat_name>` with v4 stat names |
| `nutanix_storage_container_stats_<stat>` | storage container stats | `nutanix.storage.container.stat.<stat_name>` |
| `nutanix_storage_container_usage_stats_<stat>` | storage container usage stats | `nutanix.storage.container.stat.<stat_name>` |
| `nutanix_vms_stats_<stat>` | VM stats | `nutanix.vm.stat.<stat_name>` |
| `nutanix_vms_usage_stats_<stat>` | VM usage stats | `nutanix.vm.stat.<stat_name>` |
...
snake_case keys. The receiver keeps the original source name in
`nutanix.stat.name` and appends a normalized version to the metric name. This
keeps each metric stream's measurement identity separate. Known source units
are converted to UCUM units (`By`, `us`, `{operation}/s`, or the unit ratio),
including conversion of percentages and parts-per-million to ratios. A source
stat with no recognized unit remains unitless, but is still isolated by its
metric name. Disable a complete statistic family with its generated `.stat`
