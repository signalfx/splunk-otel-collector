...
// sdkCall serializes access to a generated Nutanix client. The released SDKs
// lazily initialize and mutate their HTTP client for every request, so they
// are not safe to call concurrently even though the receiver collects
// statistics for multiple entities in parallel.
func (c *prismClient) sdkCall(call func() (any, error)) (any, error) {
	c.sdkMu.Lock()
	defer c.sdkMu.Unlock()
	return safeSDKCall(call)
}
...
func (c *prismClient) getClusterStats(ctx context.Context, cluster nutanixCluster) ([]metricStat, error) {
	...
	response, err := c.sdkCall(func() (any, error) {
		return c.clusters.GetClusterStats(ctx, &clusterRequests.GetClusterStatsRequest{
			ExtId:             &cluster.ID,
			StartTime_:        &start,
			EndTime_:          &end,
...
func (c *prismClient) getHostStats(ctx context.Context, host nutanixHost) ([]metricStat, error) {
	...
	statType := clusterStatsCommon.DOWNSAMPLINGOPERATOR_LAST
	response, err := c.sdkCall(func() (any, error) {
		return c.clusters.GetHostStats(ctx, &clusterRequests.GetHostStatsRequest{
...
func (c *prismClient) getStorageContainerStats(ctx context.Context, container nutanixStorageContainer) ([]metricStat, error) {
	...
	statType := clusterStatsCommon.DOWNSAMPLINGOPERATOR_LAST
	response, err := c.sdkCall(func() (any, error) {
		return c.storageContainers.GetStorageContainerStats(ctx, &storageContainerRequests.GetStorageContainerStatsRequest{
			ExtId:             &container.ID,
			StartTime_:        &start,
			EndTime_:          &end,
			SamplingInterval_: &sampling,
...
func (c *prismClient) listVMStats(ctx context.Context) (map[string][]metricStat, error) {
	...
	for {
		limit := v4PageSize
		response, err := c.sdkCall(func() (any, error) {
			return c.virtualMachineStats.ListVmStats(ctx, &vmStatsRequests.ListVmStatsRequest{
				StartTime_:        &start,
				EndTime_:          &end,
				SamplingInterval_: &sampling,
				StatType_:         &statType,
...
func (c *prismClient) getVolumeGroupStats(ctx context.Context, volumeGroup nutanixVolumeGroup) ([]metricStat, error) {
	...
	response, err := c.sdkCall(func() (any, error) {
		return c.volumeGroups.GetVolumeGroupStats(ctx, &volumeGroupRequests.GetVolumeGroupStatsRequest{
...
func (c *prismClient) getDiskStats(ctx context.Context, disk nutanixDisk) ([]metricStat, error) {
	...
	statType := clusterStatsCommon.DOWNSAMPLINGOPERATOR_LAST
	response, err := c.sdkCall(func() (any, error) {
		return c.disks.GetDiskStats(ctx, &diskRequests.GetDiskStatsRequest{
			ExtId:             &disk.ID,
			StartTime_:        &start,
			EndTime_:          &end,
			SamplingInterval_: &sampling,
...
func listSDKEntities(ctx context.Context, name string, fetch sdkPageFetcher) ([]map[string]any, error) {
	...
	for page := 0; ; page++ {
		...
		response, err := safeSDKCall(func() (any, error) {
			return fetch(ctx, page, v4PageSize)
		})
		if err != nil {
			return nil, err
		}
...
func safeSDKCall[T any](call func() (T, error)) (result T, err error) {
	defer func() {
		if recovered := recover(); recovered != nil {
			err = fmt.Errorf("nutanix SDK panic: %v", recovered)
		}
	}()
	return call()
}
