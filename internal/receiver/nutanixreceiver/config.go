...
type MetricsConfig struct {
	// MetricOptions contains the generated per-metric configuration. It is
	// embedded so the generated metadata configuration and the receiver's
	// category configuration can be used together under metrics:.
	metadata.MetricsConfig `mapstructure:",squash"`
...
// metricEnabled reports whether a generated metric configuration allows name.
// Statistics are emitted with a name below their generated *.stat family, so
// the family setting also controls all dynamically named statistics.
func metricEnabled(cfg metadata.MetricsConfig, name string) bool {
	value := reflect.ValueOf(cfg)
	typeOfValue := value.Type()
	for i := 0; i < value.NumField(); i++ {
		fieldName := strings.Split(typeOfValue.Field(i).Tag.Get("mapstructure"), ",")[0]
		if fieldName != name {
			continue
		}
		return value.Field(i).FieldByName("Enabled").Bool()
	}
	if separator := strings.Index(name, ".stat."); separator >= 0 {
		return metricEnabled(cfg, name[:separator+len(".stat")])
	}
	return true
}
