...
func TestSDKClientListAllPaginates(t *testing.T) {
	server := newSDKTestServer(func(w http.ResponseWriter, r *http.Request) {
...
func TestCollectAdditionalMetricsDisabledMakesNoRequests(t *testing.T) {
	var requests atomic.Int32
	server := newSDKTestServer(func(http.ResponseWriter, *http.Request) {
		requests.Add(1)
	})
	defer server.Close()
...
func TestCollectAdditionalMetricsCoversEveryOptionalDomain(t *testing.T) {
	server := newSDKTestServer(func(w http.ResponseWriter, _ *http.Request) {
		writeJSON(t, w, map[string]any{
			"data":     []any{},
...
func newSDKTestServer(handler http.HandlerFunc) *httptest.Server {
