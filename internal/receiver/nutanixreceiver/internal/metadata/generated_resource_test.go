...
func TestResourceBuilder(t *testing.T) {
	for _, tt := range []string{"default", "all_set", "none_set"} {
		t.Run(tt, func(t *testing.T) {
			...
			nutanixPrismAPIVersionAttrVal, ok := res.Attributes().Get("nutanix.prism.api.version")
			assert.True(t, ok)
			if ok {
				assert.Equal(t, "nutanix.prism.api.version-val", nutanixPrismAPIVersionAttrVal.Str())
			}
