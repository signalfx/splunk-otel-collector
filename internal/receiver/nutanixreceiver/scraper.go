...
func newScraper(settings receiver.Settings, cfg *Config) *scraper {
	return &scraper{cfg: cfg, logger: settings.Logger, buildInfo: settings.BuildInfo}
}
...
func (s *scraper) scrape(ctx context.Context) (pmetric.Metrics, error) {
	...
	builder := newMetricBuilder(s.client, s.startTime, s.cfg.APIVersion, s.buildInfo.Version, s.cfg.Metrics.MetricsConfig)
...
func (s *scraper) fetchSnapshot(ctx context.Context) (prismSnapshot, error) {
	...
	if s.cfg.Metrics.Clusters.Enabled {
		...
		if metricEnabled(s.cfg.Metrics.MetricsConfig, "nutanix.cluster.stat") {
	...
	if s.cfg.Metrics.Disks.Enabled {
		...
		if metricEnabled(s.cfg.Metrics.MetricsConfig, "nutanix.disk.stat") {
	...
	if s.cfg.Metrics.Hosts.Enabled {
		...
		if metricEnabled(s.cfg.Metrics.MetricsConfig, "nutanix.host.stat") {
	...
	if s.cfg.Metrics.StorageContainers.Enabled {
		...
		if metricEnabled(s.cfg.Metrics.MetricsConfig, "nutanix.storage.container.stat") {
	...
	if s.cfg.Metrics.VMs.Enabled || s.cfg.Metrics.DataProtection.Enabled {
		snapshot.vms, err = s.client.listVMs(ctx)
		if err != nil {
			return snapshot, fmt.Errorf("failed to list vms: %w", err)
		}
		if s.cfg.Metrics.VMs.Enabled && s.cfg.APIVersion == "v4" && metricEnabled(s.cfg.Metrics.MetricsConfig, "nutanix.vm.stat") {
	...
	if s.cfg.Metrics.VolumeGroups.Enabled {
		...
		if metricEnabled(s.cfg.Metrics.MetricsConfig, "nutanix.volume_group.stat") {
	...
	if s.cfg.Metrics.DataProtection.Enabled || s.cfg.Metrics.Files.Enabled ||
		s.cfg.Metrics.Microsegmentation.Enabled || s.cfg.Metrics.Networking.Enabled ||
		s.cfg.Metrics.Objects.Enabled || s.cfg.Metrics.PrismCentral.Enabled {
...
func fetchStatsConcurrently[T any](
	...
	for i := range entities {
		...
		go func(index int) {
			...
			value, err := safeSDKCall(func() ([]metricStat, error) {
				return fetch(ctx, entities[index])
			})
			if err == nil {
				stats[index] = value
				return
...
type metricBuilder struct {
	...
	apiVersion string
	metricCfg  metadata.MetricsConfig
	startTime  pcommon.Timestamp
	now        pcommon.Timestamp
}

func newMetricBuilder(client nutanixClient, startTime pcommon.Timestamp, apiVersion, scopeVersion string, metricCfg metadata.MetricsConfig) *metricBuilder {
...
func (b *metricBuilder) addSnapshot(snapshot prismSnapshot, cfg *Config) {
	...
	if cfg.Metrics.Hosts.Enabled {
		b.addHostMetrics(snapshot.hosts, snapshot.vms, snapshot.disks, cfg.Metrics.Disks.Enabled, cfg.Metrics.VMs.Enabled)
	}
...
func (b *metricBuilder) addHostMetrics(hosts []nutanixHost, vms []nutanixVM, disks []nutanixDisk, disksEnabled, vmsEnabled bool) {
	...
	for _, host := range hosts {
		...
		if vmsEnabled {
			b.addHostVMCounts(attrs, filterVMsByHost(vms, host))
		}
		if disksEnabled {
...
func (b *metricBuilder) addHostVMCounts(baseAttrs map[string]string, vms []nutanixVM) {
	b.addGauge("nutanix.vm.count", "Number of Nutanix VMs", "{vm}", baseAttrs, float64(len(vms)))
	for _, powerState := range []string{"on", "off"} {
		attrs := cloneAttrs(baseAttrs)
		attrs["nutanix.vm.power_state"] = powerState
	...
	for _, bus := range []string{"ide", "sata", "scsi"} {
		...
		b.addGauge("nutanix.vm.disk.count", "Number of Nutanix VM disks", "{disk}", diskAttrs, float64(countVMDisks(vms, bus)))
	}
	b.addGauge("nutanix.vm.nics.count", "Number of Nutanix VM NICs", "{nic}", baseAttrs, float64(countVMNICs(vms)))
	for _, bootType := range []string{"legacy", "uefi"} {
		stateAttrs := cloneAttrs(baseAttrs)
		stateAttrs["nutanix.vm.boot.type"] = bootType
		b.addGauge("nutanix.vm.count", "Number of Nutanix VMs", "{vm}", stateAttrs, float64(countVMsByString(vms, func(vm nutanixVM) string { return vm.BootType }, bootType)))
	}
	for _, protectionType := range []string{"unprotected", "pd_protected", "rule_protected"} {
		stateAttrs := cloneAttrs(baseAttrs)
		stateAttrs["nutanix.vm.protection.type"] = protectionType
		b.addGauge("nutanix.vm.count", "Number of Nutanix VMs", "{vm}", stateAttrs, float64(countVMsByString(vms, func(vm nutanixVM) string { return vm.ProtectionType }, protectionType)))
	}
	for _, present := range []bool{true, false} {
		stateAttrs := cloneAttrs(baseAttrs)
		stateAttrs["nutanix.vm.gpu.present"] = strconv.FormatBool(present)
		b.addGauge("nutanix.vm.count", "Number of Nutanix VMs", "{vm}", stateAttrs, float64(countVMsByBool(vms, func(vm nutanixVM) bool { return vm.HasGPU }, present)))
	}
	for _, state := range []string{"installed", "enabled", "reachable", "vss_snapshot_capable"} {
		stateAttrs := cloneAttrs(baseAttrs)
		stateAttrs["nutanix.vm.guest_tools.state"] = state
		b.addGauge("nutanix.vm.count", "Number of Nutanix VMs", "{vm}", stateAttrs, float64(countVMsWithGuestToolsState(vms, state)))
	}
}
...
func (b *metricBuilder) addEntityStats(metricName, description string, baseAttrs map[string]string, stats []metricStat) {
	...
	for _, stat := range stats {
		...
		attrs := cloneAttrs(baseAttrs)
		attrs["nutanix.stat.name"] = stat.Name
		attrs["nutanix.stat.kind"] = statKind
		name := metricName + "." + normalizeStatMetricName(stat.Name)
		unit, value := normalizeStatValue(stat.Name, stat.Value)
		b.addGauge(name, description+" Statistic: "+stat.Name, unit, attrs, value)
	}
}

func (b *metricBuilder) metricEnabled(name string) bool {
	return metricEnabled(b.metricCfg, name)
}
...
func (b *metricBuilder) addGauge(name, description, unit string, attrs map[string]string, value float64) {
	if !b.metricEnabled(name) || math.IsNaN(value) || math.IsInf(value, 0) {
		return
	}
...
func filterVMsByHost(vms []nutanixVM, host nutanixHost) []nutanixVM {
	if host.ID == "" {
		return nil
	}
	var filtered []nutanixVM
	for i := range vms {
		if vms[i].HostID == host.ID {
			filtered = append(filtered, vms[i])
		}
	}
	return filtered
