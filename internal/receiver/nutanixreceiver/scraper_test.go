...
func TestScraper(t *testing.T) {
	...
	require.InDelta(t, 42.0, findGaugeValue(t, md, "nutanix.cluster.stat.controller_num_iops", map[string]string{
	...
	require.InDelta(t, 1.0, findGaugeValue(t, md, "nutanix.storage.container.stat.controller_num_iops", map[string]string{
		"nutanix.storage.container.name": "container-a",
		"nutanix.stat.name":              "controllerNumIops",
	}), 0.001)

	require.InDelta(t, 5.0, findGaugeValue(t, md, "nutanix.volume_group.stat.controller_num_iops", map[string]string{
		"nutanix.volume_group.name": "vg-a",
		"nutanix.stat.name":         "controllerNumIOPS",
	}), 0.001)
}

func TestScraperHonorsMetricAndCategoryConfiguration(t *testing.T) {
	cfg := createDefaultConfig().(*Config)
	cfg.Endpoint = "prism.example.com"
	cfg.Username = "readonly"
	cfg.Password = "secret"
	cfg.Metrics.NutanixClusterStat.Enabled = false
	cfg.Metrics.Hosts.Enabled = false
	cfg.Metrics.StorageContainers.Enabled = false
	cfg.Metrics.VMs.Enabled = false
	cfg.Metrics.VolumeGroups.Enabled = false
	...
	md, err := s.scrape(context.Background())
	require.NoError(t, err)
	require.True(t, hasGaugeDataPoint(md, "nutanix.cluster.count", nil))
	require.True(t, hasGaugeDataPoint(md, "nutanix.cluster.info", map[string]string{
		"nutanix.cluster.name": "cluster-a",
	}))
	require.False(t, hasGaugeDataPoint(md, "nutanix.cluster.stat.controller_num_iops", nil))
	require.False(t, hasGaugeDataPoint(md, "nutanix.host.count", nil))
	require.False(t, hasGaugeDataPoint(md, "nutanix.vm.count", nil))
	require.False(t, hasGaugeDataPoint(md, "nutanix.storage.container.count", nil))
	require.False(t, hasGaugeDataPoint(md, "nutanix.volume_group.count", nil))
	require.Equal(t, 1, s.client.(*fakeNutanixClient).clusterCalls)
	require.Zero(t, s.client.(*fakeNutanixClient).hostCalls)
	require.Zero(t, s.client.(*fakeNutanixClient).storageContainerCalls)
	require.Zero(t, s.client.(*fakeNutanixClient).vmCalls)
	require.Zero(t, s.client.(*fakeNutanixClient).volumeGroupCalls)
}

func TestScraperEmitsTypedStatisticStreams(t *testing.T) {
	cfg := createDefaultConfig().(*Config)
	cfg.Endpoint = "prism.example.com"
	cfg.Username = "readonly"
	cfg.Password = "secret"
	...
	iops := findMetric(t, md, "nutanix.cluster.stat.controller_num_iops")
	require.Equal(t, "{operation}/s", iops.Unit())
	require.InDelta(t, 42, iops.Gauge().DataPoints().At(0).DoubleValue(), 0.001)
	require.Equal(t, "controllerNumIops", iops.Gauge().DataPoints().At(0).Attributes().AsRaw()["nutanix.stat.name"])

	cpu := findMetric(t, md, "nutanix.cluster.stat.hypervisor_cpu_usage_ppm")
	require.Equal(t, "1", cpu.Unit())
	require.InDelta(t, 0.009, cpu.Gauge().DataPoints().At(0).DoubleValue(), 0.000001)
}
...
func TestScraperExtendedInventoryMetrics(t *testing.T) {
	...
	require.InDelta(t, 7, findGaugeValue(t, md, "nutanix.disk.stat.disk_num_iops", map[string]string{
...
func TestScraperPrismElementUsesVMInventoryStats(t *testing.T) {
	...
	require.InDelta(t, 3, findGaugeValue(t, md, "nutanix.vm.stat.hypervisor_num_iops", map[string]string{
		"nutanix.vm.id":     "vm-1",
...
func TestScraperSkipsUnsupportedEntityStats(t *testing.T) {
	...
	require.False(t, hasGaugeDataPoint(md, "nutanix.cluster.stat.controller_num_iops", map[string]string{
		"nutanix.cluster.name": "prism-central",
	}))
}
...
func TestScraperSkipsInvalidVMStatsSelect(t *testing.T) {
	...
	require.False(t, hasGaugeDataPoint(md, "nutanix.vm.stat.hypervisor_num_iops", map[string]string{
...
type fakeNutanixClient struct {
	...
	clusterCalls          int
	hostCalls             int
	storageContainerCalls int
	vmCalls               int
	volumeGroupCalls      int
}
...
func findMetric(t *testing.T, md pmetric.Metrics, metricName string) pmetric.Metric {
	t.Helper()
