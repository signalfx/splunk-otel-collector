...
function New-MSI(
    ...
    [string]$FluentdConfig="./packaging/fpm/etc/otel/collector/fluentd/fluent.conf",
    [string]$FluentdConfDir="./packaging/msi/fluentd/conf.d"
) {
    ...
    candle -arch x64 -out "$BuildDir\splunk-otel-collector.wixobj" -dVersion="$Version" -dOtelcol="$Otelcol" .\packaging\msi\splunk-otel-collector.wxs
...
function Confirm-MSI {
    ...
    ((Get-Content -path "$configpath" -Raw) -Replace '\${SPLUNK_TRACE_URL}', 'https://ingest.us0.signalfx.com/v2/trace/otlp') | Set-Content -Path "$configpath"
