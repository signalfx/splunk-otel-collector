...
- [`Dockerfile.windows`](./Dockerfile.windows): A Windows image that installs
  ...
  The admin account is created with username `admin` and password `changeme`
  (override with `--build-arg SPLUNK_ADMIN_PASSWORD=<password>`), so the
  REST API at `localhost:8089` (mapped by `run-local-package-in-container.ps1`)
