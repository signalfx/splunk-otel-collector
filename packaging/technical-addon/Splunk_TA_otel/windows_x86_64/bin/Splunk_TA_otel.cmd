...
set "config_dir_name=config_dir"
set "config_dir_value="
...
set "splunk_discovery_properties_name=discovery_properties"
set "splunk_discovery_properties_value="
...
if "%config_dir_value%" != "" (
    set "SPLUNK_OTEL_FLAGS=%SPLUNK_OTEL_FLAGS% --config-dir=%config_dir_value%"
) else (
    call :splunk_TA_otel_log_msg "DEBUG" "Optional flag %config_dir_name% not set"
)
...
if "%splunk_discovery_properties_value%" != "" (
    set "SPLUNK_OTEL_FLAGS=%SPLUNK_OTEL_FLAGS% --discovery-properties-file=%splunk_discovery_properties_value%"
) else (
    call :splunk_TA_otel_log_msg "DEBUG" "Optional flag %splunk_discovery_properties_name% not set"
)
:: END AUTOGENERATED CODE
...
for /F "tokens=1,2 delims==" %%I in ('powershell -noninteractive -noprofile -command "$input | Select-String -Pattern '.*?(%config_dir_name%|%configd_name%|%discovery_name%|%splunk_discovery_properties_name%|%gomemlimit_name%|%splunk_access_token_file_name%|%splunk_api_url_name%|%splunk_ballast_size_mib_name%|%splunk_bundle_dir_name%|%splunk_collectd_dir_name%|%splunk_config_name%|%splunk_config_dir_name%|%splunk_config_yaml_name%|%splunk_debug_config_server_name%|%splunk_gateway_url_name%|%splunk_hec_url_name%|%splunk_ingest_url_name%|%splunk_listen_interface_name%|%splunk_memory_limit_mib_name%|%splunk_memory_total_mib_name%|%splunk_otel_log_file_name%|%splunk_realm_name%|%splunk_trace_url_name%).*?>(.*?)<' | ForEach-Object { $_.Matches.Groups[1].Value + '=' + $_.Matches.Groups[2].Value }"') do (
    if "%%I"=="%config_dir_name%" set "config_dir_value=%%J"
    if "%%I"=="%configd_name%" set "configd_value=%%J"
    if "%%I"=="%discovery_name%" set "discovery_value=%%J"
    if "%%I"=="%splunk_discovery_properties_name%" set "splunk_discovery_properties_value=%%J"
...
:: expand params in config_dir_value
for /f "delims=" %%i in ('powershell -noninteractive -noprofile -command "'%config_dir_value%' -replace '\$SPLUNK_OTEL_TA_PLATFORM_HOME', '%SPLUNK_OTEL_TA_PLATFORM_HOME%'"') do (
    set "config_dir_value=%%i"
)
for /f "delims=" %%i in ('powershell -noninteractive -noprofile -command "'%config_dir_value%' -replace '\$SPLUNK_OTEL_TA_HOME', '%SPLUNK_OTEL_TA_HOME%'"') do (
    set "config_dir_value=%%i"
)
for /f "delims=" %%i in ('powershell -noninteractive -noprofile -command "'%config_dir_value%' -replace '\$SPLUNK_HOME', '%SPLUNK_HOME%'"') do (
    set "config_dir_value=%%i"
)

:: expand params in splunk_discovery_properties_value
for /f "delims=" %%i in ('powershell -noninteractive -noprofile -command "'%splunk_discovery_properties_value%' -replace '\$SPLUNK_OTEL_TA_PLATFORM_HOME', '%SPLUNK_OTEL_TA_PLATFORM_HOME%'"') do (
    set "splunk_discovery_properties_value=%%i"
)
for /f "delims=" %%i in ('powershell -noninteractive -noprofile -command "'%splunk_discovery_properties_value%' -replace '\$SPLUNK_OTEL_TA_HOME', '%SPLUNK_OTEL_TA_HOME%'"') do (
    set "splunk_discovery_properties_value=%%i"
)
for /f "delims=" %%i in ('powershell -noninteractive -noprofile -command "'%splunk_discovery_properties_value%' -replace '\$SPLUNK_HOME', '%SPLUNK_HOME%'"') do (
    set "splunk_discovery_properties_value=%%i"
)
:: END AUTOGENERATED CODE
