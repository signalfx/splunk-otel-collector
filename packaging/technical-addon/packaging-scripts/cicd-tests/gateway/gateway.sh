...
gateway_container_info="$(splunk_orca -v --printer json --cloud $ORCA_CLOUD --ansible-log "$BUILD_DIR/ansible-local-gateway.log" create --env SPLUNK_CONNECTION_TIMEOUT=600 --platform $SPLUNK_PLATFORM --local-apps "$GATEWAY_TA_FULLPATH" --playbook "$SOURCE_DIR/packaging-scripts/orca-playbook-windows.yml,site.yml")"
...
gateway_agent_container_info=$(splunk_orca -v --printer json --cloud $ORCA_CLOUD --ansible-log "$BUILD_DIR/ansible-local-gateway-agent.log" create --env SPLUNK_CONNECTION_TIMEOUT=600 --platform $SPLUNK_PLATFORM  --local-apps "$GATEWAY_AGENT_TA_FULLPATH" --playbook "$SOURCE_DIR/packaging-scripts/orca-playbook-windows.yml,site.yml")
GATEWAY_AGENT_IPV4_ADDR="$(echo "$gateway_agent_container_info" | jq -r '.[keys[0]] | .[keys[0]] | .containers | .[keys[0]] | .ssh_address')"

echo "gateway container info:"
echo "$gateway_container_info"
echo "gateway agent container info:"
echo "$gateway_agent_container_info"
GATEWAY_LOGS_DIR="$BUILD_DIR/orca-test-logs/gateway/$GATEWAY_REPACKED_TA_NAME/"
mkdir -p "$GATEWAY_LOGS_DIR"
GATEWAY_AGENT_LOGS_DIR="$BUILD_DIR/orca-test-logs/gateway/$GATEWAY_AGENT_REPACKED_TA_NAME/"
