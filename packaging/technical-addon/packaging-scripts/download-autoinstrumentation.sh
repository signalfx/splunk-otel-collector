...
if [ "$PLATFORM" == "linux" ] || [ "$PLATFORM" == "all" ]; then
  ...
  wget --no-verbose  --timestamping "https://github.com/signalfx/splunk-otel-js/releases/download/${NODEJS_VERSION}/splunk-otel-${NODEJS_VERSION#v}.tgz" --output-document "$nodejs_agent_path"
  wget --no-verbose  --timestamping "https://github.com/signalfx/splunk-otel-java/releases/download/${JAVA_VERSION}/splunk-otel-javaagent.jar" --output-document  "$java_agent_path"
  # Needed for go:embed
