			...
			func(t *testing.T) ptrace.Traces {
				traces := newPDataSpan(
					...
					map[string]any{
						"some tag":              "some tag value",
						"another tag":           "another tag value",
						"network.local.address": loopback,
						"net.host.port":         100,
						"network.peer.address":  linkLocal,
						"net.peer.port":         200,
						"service.peer.name":     anotherServiceName,
					},
					map[uint64]string{
			...
			func(t *testing.T) ptrace.Traces {
				traces := newPDataSpan(
					...
					map[string]any{
						"some tag":              "some tag value",
						"another tag":           "another tag value",
						"network.local.address": loopbackIPv6,
						"net.host.port":         100,
						"network.peer.address":  linkLocalIPv6,
						"net.peer.port":         200,
						"service.peer.name":     anotherServiceName,
					},
					map[uint64]string{
