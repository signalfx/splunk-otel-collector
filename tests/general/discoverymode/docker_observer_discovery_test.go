...
func TestDockerObserver(t *testing.T) {
	...
	_, shutdownPrometheus := tc.Containers(
		testutils.NewContainer().WithImage("quay.io/prometheus/prometheus:v3.13.1").WithLabel("test.id", tc.ID).WillWaitForLogs("Server is ready to receive web requests."),
	)
	...
	cc, shutdown := tc.SplunkOtelCollectorContainer(
		...
		func(c testutils.Collector) testutils.Collector {
			return c.WithEnv(map[string]string{
				...
				"--set", `splunk.discovery.receivers.prometheus/test.enabled=true`,
