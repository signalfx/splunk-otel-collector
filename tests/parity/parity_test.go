...
// update regenerates the checked-in golden files from the UF oracle instead of
// replaying against them. It needs a UF install (PARITY_UF_DIR); a normal run
// does not.
...
// TestParity replays each case: it runs the otelcol candidate into a real Splunk
// and asserts the events it lands match the case's checked-in golden on the
// fields the case asserts. The golden is the single source of truth and is
// generated from the UF oracle out of band with -update, so a normal run (and CI)
// needs only the collector and Splunk, not a UF install.
func TestParity(t *testing.T) {
	...
	for _, path := range cases {
		...
		t.Run(c.Name, func(t *testing.T) {
			...
			if *update {
				regenerateGolden(ctx, t, c, backend, goldenPath, opts)
			}
...
// regenerateGolden runs the UF oracle, projects its events down to the fields
// the case asserts, and writes the golden. UF is the authority the golden is
// captured from; it is exercised only here, not on a normal replay.
func regenerateGolden(ctx context.Context, t *testing.T, c *parity.Case, backend parity.Backend, goldenPath string, opts parity.RunOptions) {
	t.Helper()
	...
	if err := parity.WriteGolden(goldenPath, parity.ProjectAll(ufRecs, c.Assert)); err != nil {
		t.Fatalf("write golden: %v", err)
	}
	t.Logf("regenerated %s from UF (%d record(s), fields %v)", goldenPath, len(ufRecs), c.Assert)
}
