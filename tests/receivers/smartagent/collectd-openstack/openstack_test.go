...
// TestCollectdOpenstackReceiver tests that the collectd/openstack monitor emits the expected
// metrics when connected to a live devstack OpenStack deployment running on the host machine
// (deployed via gophercloud/devstack-action in CI).
func TestCollectdOpenstackReceiver(t *testing.T) {
	// ATTENTION: currently Cinder metrics are NOT being tested since it seems that the monitor code is too old
	// and has not been updated since Cinder switched to block-storage vs volumev2 service type. This needs
	// to be investigated as a separate issue. For now, tests of "all" and "default" metrics do not include
	// Cinder metrics.
	testCases := []struct {
		name       string
		configFile string
		...
		{
			name:       "DefaultMetrics",
			configFile: "default_metrics_config.yaml",
			expected:   "default.yaml",
		},
		{
			name:       "AllMetrics",
			configFile: "all_metrics_config.yaml",
			expected:   "all.yaml",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, shutdown := testcase.SplunkOtelCollectorProcess(tc.configFile)
			defer shutdown()

			expected, err := golden.ReadMetrics(filepath.Join("testdata", "expected", tc.expected))
