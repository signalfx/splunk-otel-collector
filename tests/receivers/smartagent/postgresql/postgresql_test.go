...
func TestPostgresReceiverProvidesAllMetrics(t *testing.T) {
	...
	).WithExposedPorts("5432:5432").WithName("postgres-server").WithNetworkLabels(
		"postgres",
	...
	).WithName("postgres-client").WithNetworkLabels("postgres").WillWaitForLogs("Beginning psql requests")
